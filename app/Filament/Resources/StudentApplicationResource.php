...
class StudentApplicationResource extends Resource
{
    ...
    protected static ?string $label = 'Student Applications';
        ...
        return $table
            ->modifyQueryUsing(function (Builder $query) {
                if (!Auth::user()->hasRole('Admin')) {
                    $query->whereHas('collegeApplication', function (Builder $query) {
                        $query->where('allocated_user', Auth::user()->id);
                    });
                } else {
                    $query->whereHas('collegeApplication');
                }
            })
            ->columns([
    ...
    public static function getPages(): array
    {
        return [
            ...
            'edit' => \App\Filament\Resources\StudentResource\Pages\EditStudent::route('../students/{record}/edit?activeRelationManager=2'),
        ];
    }
